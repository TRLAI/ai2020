# -*- coding: utf-8 -*-
"""
Created on Mon Oct 26 23:06:22 2020
...
class SplitDataset():
    ...
    def __get_all_file_path(self):
        all_file_path = []
        index = 0
        for file_type in self.__get_label_names():  # 得到各个类别
            self.index_label_dict[index] = file_type  # 给每个类别一个索引
            index += 1
            type_file_path = os.path.join(self.dataset_dir, file_type)  # 当前类别路径
            file_path = []
            for file in os.listdir(type_file_path):  # 遍历当前类别里的所有样本
    ...
    def __split_dataset(self):
        ...
        for index in range(len(all_file_paths)):  # 每个类别分别做划分
            file_path_list = all_file_paths[index]  # 当前类别所有样本路径
