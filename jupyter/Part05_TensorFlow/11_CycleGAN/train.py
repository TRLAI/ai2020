...
@tf.function
def train_step(G, F, D_Y, D_X, source, target, generator_g_optimizer, generator_f_optimizer, discriminator_x_optimizer,
               discriminator_y_optimizer, train_loss, lambda1, lambda2):
    with tf.GradientTape(persistent=True) as tape:
        ...
        # cycle的损失，针对G的source和cycled_x越接近越好，针对F的target和cycled_y越接近越好
        total_cycle_loss = tf.reduce_mean(tf.abs(source - cycled_x)) * lambda1 + tf.reduce_mean(
            tf.abs(target - cycled_y)) * lambda2
        # 加上第三部分
        ...
        # 判别器的损失，真的判别为真，假的判别为假(0是默认，可以不写)
...
def main(_argv):
    ...
    for ep in range(start, epoch, 1):
        ...
        for step, source in enumerate(source_data):
            ...
            fake_y, cycled_x, fake_x, cycled_y, same_x, same_y, total_gen_g_loss, total_gen_f_loss, disc_x_loss, disc_y_loss, steps = train_step(
                G, F, D_Y, D_X, source, target, generator_g_optimizer, generator_f_optimizer, discriminator_x_optimizer,
                discriminator_y_optimizer, train_loss, lambda1, lambda2)
            print('Step: ' + str(steps.numpy()) + ' , G loss: ' + str(total_gen_g_loss.numpy()) + ' , F loss: ' + str(
                total_gen_f_loss.numpy()) + ' , D_X loss: ' + str(disc_x_loss.numpy()) + ' , D_Y loss: ' + str(
                disc_y_loss.numpy()))
